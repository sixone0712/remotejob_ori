...
export default function BuildHistoryMenu({}: BuildHistoryMenuProps): JSX.Element {
  ...
  useEffect(() => {
    if (fromScheduelDataDoneRef.current && renderHistory && menuRef.current) {
      menuRef.current.scrollIntoView({ block: 'center' });
      window.window.scrollTo({ top: 0, left: 0, behavior: 'auto' });
      fromScheduelDataDoneRef.current = false;
    }
  }, [renderHistory, fromScheduelDataDoneRef.current, menuRef.current]);
