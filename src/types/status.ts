...
export type BuildStatus = 'success' | 'failure' | 'notbuild' | 'processing' | 'canceled' | 'none' | 'nodata';
...
export type StatusStepType = 'convert' | 'collect' | 'error' | 'cras' | 'version' | 'purge';
...
export type LocalColumnName = 'index' | 'companyFabName' | 'collectStatus' | 'files' | 'registeredDate' | 'delete';
...
export type LocalColumnPropsType = {
  [name in LocalColumnName]: {
    ...
    shouldCellUpdate?: (record: LocalStatus, prevRecord: LocalStatus) => boolean;
  };
};
